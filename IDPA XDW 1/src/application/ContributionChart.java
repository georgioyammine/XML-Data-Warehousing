...
/**
 * User: hansolo Date: 23.10.17 Time: 03:25
 */
public class ContributionChart extends Application {
	...
	public void init() {
		...
		int lastDay = (int) now.getDayOfWeek().getValue();
		int daysInMonth = YearMonth.of(year - 1, month).lengthOfMonth();
		System.out.println(now);

		LocalDate endDate = LocalDate.now().plusDays(7 - LocalDate.now().getDayOfWeek().getValue());
		LocalDate startDate = endDate.minusYears(1).plusDays(7 - endDate.minusYears(1).getDayOfWeek().getValue());
		...
		matrix.setOnDotMatrixEvent(e -> {
			...
			Data selectedData = dataList.stream().filter(data -> data.x == e.getX() && data.y == e.getY()).findFirst()
					.orElse(null);
			if (selectedData != null && selectedData.getDate() != null &&selectedData.getDate().isBefore(now)) {
		...
		legend = new HBox(5, new Text("Less"), new Rectangle(10, 10, colorCoding.get(0.0)),
				new Rectangle(10, 10, colorCoding.get(2.0)), new Rectangle(10, 10, colorCoding.get(4.0)),
				new Rectangle(10, 10, colorCoding.get(6.0)), new Rectangle(10, 10, colorCoding.get(8.0)),
				new Text("More"));
