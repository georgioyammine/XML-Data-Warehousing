...
import com.georgioyammine.controllers.dataWarehousingController;
...
public class DataWarehousing implements Serializable {
	...
	public boolean addNewVersion(String owner, String pathOfNewVersion) throws Exception {
		...
		if (versions.isEmpty()) {
			...
			String relativePath = File.separator + "src" + File.separator + "available files" + File.separator + name
					+ "_v1" + ".xml";
			...
			Files.copy(Paths.get(file.getAbsolutePath()),
					Paths.get(path + File.separator + name + "_v" + (versions.size() + 1) + ".xml"));
			File copied = new File(path + File.separator + name + "_v" + (versions.size() + 1) + ".xml");
			// String relativePath = File.separator + "src"+ File.separator+"available
			// files"+File.separator+name+"_v"+(versions.size()+1);
			...
			String relativePath = "src" + File.separator + "available files" + File.separator + name + "_v"
					+ (versions.size() + 1) + ".xml";

			Node diff = (Node) TED.get(0);
			String diffPath = XMLDiffAndPatch.WriteXMLtoFile2(diff,
					"Diff_" + (versions.size() + 1) + "to" + (versions.size()) + ".xml", diffFolder, true);
			versions.add(new Version(versions.size() + 1, owner, currentDate, copied.length(), relativePath,
					((double) TED.get(1)) * 100, true, new File(diffPath).length()));
	...
	public static DataWarehousing load(String filename) {
		...
		try {
			...
			input = (DataWarehousing) ois.deserialize(new FileInputStream(filename));
		} catch (Exception e) {
			e.printStackTrace();
		}
		return input;
	...
	public boolean getThisVersion(Version version) throws Exception {
		...
		String diffPath = XMLDiffAndPatch.WriteXMLtoFile2(node, name + "_v" + (version.getNumber()) + ".xml",
				outputFolder, true);
	...
	public Node getThisVersionAsNode(Version version) throws Exception {
		...
		if (version.getNumber() != versions.size()) {
			String diffFile = diffFolder + File.separator + "Diff_" + (version.getNumber() + 1) + "to"
					+ (version.getNumber()) + ".xml";
